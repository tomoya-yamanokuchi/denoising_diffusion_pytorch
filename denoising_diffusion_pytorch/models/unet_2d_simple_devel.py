...
class Unet(nn.Module):
    def __init__(
        ...
        input_channels = 7 * (2 if self_condition else 1)
    ...
    def forward(self, x, time, x_self_cond = None, mask_cond = None, cfg = False):
        assert all([divisible_by(d, self.downsample_factor) for d in x.shape[-2:]]), f'your input dimensions {x.shape[-2:]} need to be divisible by {self.downsample_factor}, given the unet'

        if mask_cond is not None: 
            if cfg == False:    
                mask_label = (mask_cond != -1.0).any(dim=1, keepdim=True).float() # tmp13以降
                # mask_label = (mask_cond != -1.0).all(dim=1, keepdim=True).float()  # tmp13以前                
            elif cfg == True:
                mask_label = (mask_cond != -1.0).any(dim=1, keepdim=True).float()*0.0 # tmp13以降
