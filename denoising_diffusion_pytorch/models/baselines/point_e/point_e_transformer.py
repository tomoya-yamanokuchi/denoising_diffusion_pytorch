...
class PointDiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.time_embed = MLP(
            device=device_name, dtype=dtype, width=width, init_scale=init_scale * math.sqrt(1.0 / width)
        )
        self.ln_pre = nn.LayerNorm(width, device=device_name, dtype=dtype)
        ...
        self.ln_post = nn.LayerNorm(width, device=device_name, dtype=dtype)
        self.input_proj = nn.Linear(input_channels, width, device=device_name, dtype=dtype)
        self.output_proj = nn.Linear(width, output_channels, device=device_name, dtype=dtype)
        with torch.no_grad():
...
if __name__ == '__main__':
    B, N = 8, 1024  # バッチサイズ、点の数
    x = torch.randn(B, 6, N)      # 入力：6次元（x,y,z,r,g,b）特徴を T=1024 点に対して
    t = torch.randint(0, 1000, (B,))  # 拡散ステップ

    model = PointDiffusionTransformer(
        device_name=torch.device('cuda'),
        dtype=torch.float32,
        input_channels=6,     # 入力次元：点の特徴
        output_channels=6,    # 出力も同じ（εの予測）
        n_ctx=1024,           # 1サンプルあたりのトークン数（点数や系列長）
