...
from denoising_diffusion_pytorch.models.helpers import default,normalize_to_neg_one_to_one,unnormalize_to_zero_to_one,identity,extract,linear_beta_schedule,cosine_beta_schedule,sigmoid_beta_schedule
...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, x_self_cond = None, mask = None, cfg = False, clip_x_start = False, rederive_pred_noise = False):
        if mask is not None:
            if cfg == False:
                model_output = self.model(x, t, x_self_cond, mask)
            elif cfg == True:
    ...
    @torch.inference_mode()
    def ddim_sample(self, shape, return_all_timesteps = False , cond = None, mask = None):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            if cond is not None:
                ...
                single_image_shape = cond[0]["val"].shape
                x_known  = sqrt_alphas_cumprod_t*x_0_org + sqrt_one_minus_alphas_cumprod_t*torch.randn(single_image_shape, device=device)
                cond[0]["val"] = x_known # cond[0]["val"] = x_known.clamp(-1,1)
                img = apply_conditioning(x = img ,cond=cond)
            else:
                pass
    ...
    @torch.inference_mode()
    def sample(self, batch_size = 16, return_all_timesteps = False , cond = None, mask = None):
        image_size, channels = self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, image_size, image_size), return_all_timesteps = return_all_timesteps , cond  = cond, mask = mask)
    ...
    def p_losses(self, x_start, t, noise = None, offset_noise_strength = None, mask=None):
        ...
        # x = binary_mask * mask + (1 - binary_mask) * x
        ...
        # binary_mask = (mask != -1.0).any(dim=1, keepdim=True).float()  # → [B, 1, H, W]
