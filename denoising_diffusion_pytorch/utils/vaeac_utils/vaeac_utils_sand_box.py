...
from denoising_diffusion_pytorch.utils.vaeac_utils.celeba import CelebA
# from celeba import CelebA

from denoising_diffusion_pytorch.utils.vaeac_utils.celeba_vaeac import EncoderDecoder
# from celeba_vaeac import EncoderDecoder as celeba_vaeac
# from networks import mnist_vaeac, celeba_vaeac

from denoising_diffusion_pytorch.utils.vaeac_utils.losses import MSERecon,BCERecon
...
def save_ckpt(model_and_optim, cfg, ckpt, seed, override=False , save_path = None):
    '''
    Save the checkpoint which is described by `ckpt`, and parameters, and also the seed used
    for reproducibility
    '''
    model, optim = model_and_optim
    if override or (ckpt) % cfg['train']['peek-validation'] == 0:
        state_dict = dict()
...
def save_val_images(data, outs, cfg, save_index, suffix='val'):
    ...
    if cfg['model']['inp_channels'] == 1:
        ...
        # Save it
        io.imsave('{}/{}_val_out_{}.png'.format(cfg['save-path'], save_index, suffix),\
            (((out_img + 1.0)/2.0)*255).astype(np.uint8))
        print("Saved for ckpt: {} {}".format(save_index, suffix))
