...
Key behavior
------------
- Supports both single-product and multi-product I2SB datasets.
- At initialization, chooses fixed validation examples per product.
- The X0 / X1 / condition / mask used for checkpoint visualization are
  cached once, so progress images remain directly comparable over training.
- At every `save_and_sample_every` milestone, the EMA model performs I2SB
  reverse sampling for every cached product example.
- Saves both:
    1. combined grids across all products, and
    2. per-product grids.
- Existing file naming is preserved for the combined grids:
    sample-{step}_pred.png
    sample-{step}_mask.png
    sample-{step}_target.png
    sample-{step}_cond.png
    sample-{step}_x1.png
"""
...
class Trainer:
    def __init__(
        ...
        *,
        ...
        num_samples=4,
        # Product-aware logging settings.
        ...
        **unused_kwargs,
        ...
        # ------------------------------------------------------------------
        # Periodic visualization configuration
        # ------------------------------------------------------------------
        ...
        if len(dataset) < 2:
            raise ValueError(
                "I2SB training requires at least 2 "
        ...
        # ------------------------------------------------------------------
        # Build fixed product-aware visualization samples BEFORE wrapping
        # the dataloaders with Accelerator.
        #
        # We use indices from the validation split, then evaluate
        # `dataset[idx]` once and cache the returned tensors. This keeps:
        #   - X0 fixed
        #   - X1 fixed
        #   - condition fixed
        #   - mask fixed
        # across all checkpoints in a run.
        # ------------------------------------------------------------------
        self.log_samples = (
            self._build_fixed_log_samples(
                val_dataset
            )
        )
    ...
    # ==================================================================
    # Product-aware fixed log sample preparation
    # ==================================================================

    def _build_fixed_log_samples(
        self,
        val_dataset,
    ):
        """
        Select `samples_per_product` examples from the validation subset
        for every product and cache their tensors.

        The dataset is expected to expose:
            dataset.samples[i]["product_name"]

        This is provided by the new single/multi-product
        ...
        if not hasattr(
            ...
            raise AttributeError(
                "Product-aware I2SB logging requires "
                "dataset.samples metadata."
            )

        product_order = [
            product["name"]
            for product in getattr(
                self.dataset,
                "products",
                [],
            )
        ...
        # random_split returns Subset, whose `.indices` refer to the
        # original dataset.
        for dataset_idx in (
            val_dataset.indices
        ):
        ...
        if missing:
            raise RuntimeError(
                "Validation split does not contain "
                "enough fixed logging samples for: "
                f"{missing}. "
                "Increase validation size or reduce "
                "samples_per_product."
            )
        ...
        for product_name in product_order:
            for sample_id, dataset_idx in enumerate(
                ...
                # Cache CPU tensors once. This also freezes the random
                # mask / condition for checkpoint-to-checkpoint comparison.
                log_samples.append(
                    {
                        "product_name":
                            product_name,
    ...
    def _print_log_sample_summary(
        self,
    ):
        self.accelerator.print(
            "[I2SB] fixed product-aware "
            "validation samples:"
        )
    ...
    def compute_loss(
        ...
        if (
            ...
            raise RuntimeError(
                "Network prediction and I2SB "
                "target must have identical "
                "shapes, got "
    ...
    # ==================================================================
    # Periodic reverse-sampling helpers
    # ==================================================================
    ...
    def sample_and_log(
        ...
        # --------------------------------------------------------------
        # Concatenate all products for backward-compatible combined logs
        # --------------------------------------------------------------
        all_pred = torch.cat(
            [
                result["pred"]
        ...
        pred_vis = self._to_01(
            all_pred
        )
        target_vis = self._to_01(
            all_target
        )
        cond_vis = self._to_01(
            all_cond
        )
        x1_vis = self._to_01(
            all_x1
        ...
        # Make rows naturally align with products when one sample/product
        # is used. For >1 sample/product, each row can contain one product.
        nrow = max(
            1,
            self.samples_per_product,
        )
        ...
        # --------------------------------------------------------------
        # Combined files: preserve existing filenames
        # --------------------------------------------------------------
        utils.save_image(
            pred_vis,
            str(
                self.results_folder
                / f"sample-{milestone}_pred.png"
            ),
            nrow=nrow,
        )
        ...
        utils.save_image(
            ...
            str(
                self.results_folder
                / f"sample-{milestone}_target.png"
            ),
            nrow=nrow,
        )
        ...
        # Return EMA model to train mode to match the existing behavior.
        self.ema.ema_model.train()

        self.accelerator.print(
            "[I2SB] product-aware samples saved to "
            f"{milestone_dir}"
        )
            ...
            while (
                ...
                if (
                    ...
                    if (
                        ...
                        # 1) EMA reverse-sampling progress log.
                        self.sample_and_log(
                            milestone
                        )
