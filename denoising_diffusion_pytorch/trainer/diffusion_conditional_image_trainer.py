...
from denoising_diffusion_pytorch.models.helpers import exists,has_int_squareroot,cycle,divisible_by,num_to_groups
                    ...
                    # if True:
                        ...
                        with torch.inference_mode():
                            ...
                            for n in range(self.num_samples):
                                ...
                                images = self.ema.ema_model.sample(batch_size=1,mask=mask)
                        ...
                        utils.save_image(all_images, str(self.results_folder / f'sample-{milestone}_pred.png'), nrow = int(math.sqrt(self.num_samples)))
                        utils.save_image(all_masks, str(self.results_folder / f'sample-{milestone}_mask.png'), nrow = int(math.sqrt(self.num_samples)))
                        utils.save_image(all_targets, str(self.results_folder / f'sample-{milestone}_target.png'), nrow = int(math.sqrt(self.num_samples)))


                        self.writer.add_images('my_image_batch', all_images, self.step,  dataformats='NCHW')
                        self.writer.add_images('my_mask_batch', all_masks, self.step,  dataformats='NCHW')
                        self.writer.add_images('my_target_batch', all_targets, self.step,  dataformats='NCHW')
