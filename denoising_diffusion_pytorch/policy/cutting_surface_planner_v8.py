...
class cutting_surface_planner():
    ...
    def __init__(self, diffusion, trainer,sample_image_num, obs_model, observation_mode, config):
        ...
        self.total_samples = self.sample_image_num
    ...
    def get_slice_range(self,cost,axis,observation_history):
        ...
        if self.observation_mode == "both_cs_obs":
            top_slice_range = np.arange(offset, offset+top).tolist()
            btm_slice_range = np.arange(offset+btm, offset+cost.shape[0]).tolist()
    ...
    def get_opposite_site_cutting_surface(self, env2, action_idx):
        obs,reward,done,info = env2.step(action_idx=action_idx, partial_obs = {})
    ...
    def get_optimal_act(self,slice_img_,observation_history, env2,tmp_action,iters,save_path):
        ...
        ####################################################################################
        # if self.observation_mode == "both_cs_obs":
        #     obs,reward,done,info = env2.step(action_idx=tmp_action, partial_obs = {})
        # else:
        ...
        model_img_train_dim = 344
        # model_img_train_dim = 64
        ...
        if self.policy_config["ctrl_mode"] != "oracle_obs" and self.policy_config["ctrl_mode"] != "random" :
            ...
            if self.policy_config["infer_model"] == "vaeac":
                ...
                ## last_step_images1 = self.infer_image_by_vaeac(normalized_cond=normalized_cond)
                ## last_step_images2 = self.infer_image_by_vaeac(normalized_cond=normalized_cond)
                ## last_step_images  = np.concatenate((last_step_images1,last_step_images2),axis=0)
                if  torch.cuda.get_device_name() != "NVIDIA H100 NVL":
                    samples_per_iter = 4
                else:
                    samples_per_iter = 2
    
                num_iterations = self.total_samples // samples_per_iter
                self.sample_image_num = samples_per_iter
                all_images = []
                for _ in range(num_iterations):
                    images = self.infer_image_by_vaeac(normalized_cond=normalized_cond)
                    all_images.append(images)

                last_step_images = np.concatenate(all_images, axis=0)
                ...
                if  torch.cuda.get_device_name() != "NVIDIA H100 NVL":
                    ...
                    all_images = []
                    for _ in range(num_iterations):
                        images = self.infer_image_by_conditional_diffusion(normalized_cond=normalized_cond)
                        all_images.append(images)
                    last_step_images = np.concatenate(all_images, axis=0)
                else:
                ...
                if  torch.cuda.get_device_name() != "NVIDIA H100 NVL":
                    ...
                    all_images = []
                    for _ in range(num_iterations):
                        images = self.infer_image_by_diffusion_1D(slice_img)
                        all_images.append(images)
                    last_step_images = np.concatenate(all_images, axis=0)
                else:
                    last_step_images = self.infer_image_by_diffusion_1D(slice_img)
