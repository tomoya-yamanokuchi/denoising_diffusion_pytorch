...
def resize_imgs_origin_color(img,tgt_img_size):
    
    img_resize = img.resize(tgt_img_size)
    
    img_resize_np = np.asarray(img_resize)/255

    mask_config = {"image_mask_config_b" : {"target_mask"  :np.asarray([0.2,0.8,0.8]),
                    ...
                    "image_mask_config_r": {"target_mask"   :np.asarray([0.9,0.2,0.2]),
                                            "target_mask_lb":np.asarray([0.9,0.2,0.2])-np.asarray([0.1,0.1,0.1]),
                                            "target_mask_ub":np.asarray([0.9,0.2,0.2])+np.asarray([0.2,0.2,0.2])}, #np.asarray([0.2,0.6,0.6])
                    ...
                    "image_mask_config_g": {"target_mask"   :np.asarray([0.9,0.9,0.9]),
                                            "target_mask_lb":np.asarray([0.9,0.9,0.9])-np.asarray([0.05,0.05,0.05]),
                                            "target_mask_ub":np.asarray([0.9,0.9,0.9])+np.asarray([0.05,0.05,0.05])},
                    "image_mask_config_black": {"target_mask"   :np.asarray([0.0,0.0,0.0]),
                                            "target_mask_lb":np.asarray([0.0,0.0,0.0])-np.asarray([0.0,0.0,0.0]),
                                            "target_mask_ub":np.asarray([0.0,0.0,0.0])+np.asarray([0.5,0.5,0.5])},
                    "image_mask_config_white": {"target_mask"   :np.asarray([1.0,1.0,1.0]),
                                            "target_mask_lb":np.asarray([1.0,1.0,1.0])-np.asarray([0.09,0.09,0.09]),
                                            "target_mask_ub":np.asarray([1.0,1.0,1.0])+np.asarray([0.,0.,0.])}
                    }


    # masked_image = color_mask(img_resize_np,mask_config["image_mask_config_r"])
    # masked_image = color_mask(masked_image,mask_config["image_mask_config_b"])
    # masked_image = color_mask(masked_image,mask_config["image_mask_config_y"])
    masked_image = color_mask(img_resize_np,mask_config["image_mask_config_black"])
    masked_image = color_mask(masked_image,mask_config["image_mask_config_white"])
...
class cutting_surface_planner():
    ...
    def infer_image_by_diffusion(self,normalized_cond):
        ...
        if self.policy_config["ctrl_mode"] == "no_cond":
            ...
            mask = (normalized_cond > -1.0).all(dim=0)  # → shape: [256, 256] のマスク
            cond = {
                0: {
    ...
    def get_optimal_act(self,slice_img_,observation_history, env2,tmp_action,iters,save_path):
        ...
        model_img_train_dim = (self.diffusion.model.image_size,self.diffusion.model.image_size)
        # model_img_train_dim = None
        
        if model_img_train_dim is not None:
            # NumPy → PIL
            ...
            ## kwon rsesize ver
            ####################
            # slice_img = cv2.cvtColor(resize_imgs_origin_color(cv2.cvtColor(np.asanyarray(img_pil), cv2.COLOR_RGB2BGR),tgt_img_size=(model_img_train_dim,model_img_train_dim)),cv2.COLOR_BGR2RGB)/255

            ############################
            ## segmentation baed resize ver
            #############################
            # slice_img = resize_imgs_origin_color(img_pil,model_img_train_dim)

            ########################
            ## pytorch resize ver
            #########################
            transform = transforms.Compose([
                transforms.Resize((model_img_train_dim,model_img_train_dim),interpolation=InterpolationMode.NEAREST),
                # transforms.Resize((model_img_train_dim,model_img_train_dim)),
            ])
            slice_img = np.asarray((transform(img_pil)))


            # transform = transforms.Compose([
            #     transforms.Resize((model_img_train_dim,model_img_train_dim),interpolation=InterpolationMode.NEAREST),
            # ])
            # slice_img = np.asarray(transform(img_pil))/255
            
            
            # transform = transforms.Compose([
            #     transforms.Resize((int(model_img_train_dim*8),int(model_img_train_dim)*8),interpolation=InterpolationMode.BICUBIC),
            #     transforms.Resize((model_img_train_dim,model_img_train_dim),interpolation=InterpolationMode.NEAREST),
            # ])
            # slice_img = np.asarray(transform(img_pil))/255
            
            
            pil_image_save_from_numpy(slice_img/255.0,f"{cond_ds_image_save_path}/seq_obs_cast_{iters}_axis_z_{0}.png")
