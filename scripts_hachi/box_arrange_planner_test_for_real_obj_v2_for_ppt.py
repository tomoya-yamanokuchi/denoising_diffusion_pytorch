...
if __name__ == '__main__':
    ...
    config_name    = f"config/SheetSander_{tag}.yaml"

    with open(os.path.join(dataset_path, config_name), encoding='utf-8')as f:
        model_config_= yaml.safe_load(f)

    save_root_folder = dataset_path+"/random_generation_v2/tmptmp/"
    ...
    # import ipdb;ipdb.set_trace()
    mesh0        = pv.read("/home/haxhi/workspace/denoising_diffusion_pytorch/scripts/visualization/assets/cuttBody1.stl")
    mesh1        = pv.read("/home/haxhi/workspace/denoising_diffusion_pytorch/scripts/visualization/assets/cuttBody2.stl")
    mesh2        = pv.read("/home/haxhi/workspace/denoising_diffusion_pytorch/scripts/visualization/assets/cuttBody3.stl")
    mesh3        = pv.read("/home/haxhi/workspace/denoising_diffusion_pytorch/scripts/visualization/assets/cuttBody4.stl")
    mesh4        = pv.read("/home/haxhi/workspace/denoising_diffusion_pytorch/scripts/visualization/assets/cuttBody5.stl")
    mesh5        = pv.read("/home/haxhi/workspace/denoising_diffusion_pytorch/scripts/visualization/assets/cuttBody6.stl")
    mesh6        = pv.read("/home/haxhi/workspace/denoising_diffusion_pytorch/scripts/visualization/assets/cuttMotor.stl")
    # del model_config["internal_parts"]["Motor"]
    ...
    # plotter.open_movie(f"./real_model_arrange_{tag}"+".mp4")
    plotter.open_gif(f"./real_model_arrange_{tag}"+".gif")
    ...
    view_offset_val = 0.05 # render_1
    plotter.camera.position = (0.5+-view_offset_val, -0.4+view_offset_val, 0.7+view_offset_val) # polisher #latest
    plotter.camera.up = (0.0, -1.0, 0.0) #latst
    ...
    for idx, val in enumerate(model_config):
        for idx, vals in enumerate(model_config[val]):
            ...
            if val == "outer_parts":
                ...
                # plotter.add_mesh(mesh0, opacity=0.5, color = color,show_edges=False,)
                # plotter.add_mesh(mesh1, opacity=0.5, color = color,show_edges=False,)
                # plotter.add_mesh(mesh2, opacity=0.5, color = color,show_edges=False,)
                # plotter.add_mesh(mesh3, opacity=0.5, color = color,show_edges=False,)
                # plotter.add_mesh(mesh4, opacity=0.5, color = color,show_edges=False,)
                # plotter.add_mesh(mesh5, opacity=0.5, color = color,show_edges=False,)
                pass
            else:
                if vals!="Motor":
                    plotter.add_mesh(mesh, opacity=0.8, color = color,show_edges=False,)
                pass
    plotter.add_mesh(mesh6, opacity=0.8, color = [1.0,0,0],show_edges=False,)
