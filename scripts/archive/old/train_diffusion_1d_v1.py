...
if __name__ == '__main__':
    ...
    # after a lot of training
        ...
        cond_image_1d_tmp = to_torch(torch.rand(10,6,4096))
        # cond_image_1d_tmp = torch.ones(10,6,4096)
        # cond_image_1d_tmp = torch.zeros(10,6,4096)+1.0
        ...
        # voxel_indices = cond_image_1d[:3,:]
        # voxel_values  = cond_image_1d[3:,:]
        ...
        cond_image_1d = torch.cat((voxel_indices,voxel_values), dim=0)

        cond = {0:{ "idx":torch.where(voxel_values.mean(0)>0.0),
                    "val":voxel_values,
                    "pos":voxel_indices,
                    "data":cond_image_1d,
                    "datas":torch.cat((voxel_indices,voxel_values), dim=0)}}

        # aa = torch.where(cond_image_1d[3:,:].mean(0)>0.0,cond_image_1d,cond_image_1d_tmp[0])
        # aa = torch.where(cond_image_1d[3:,:].mean(0)>0.0,0.0,cond_image_1d_tmp)
        bb = torch.where(cond_image_1d[3:,:].mean(0)>-1.0,cond_image_1d,cond_image_1d_tmp)
        ...
        #     # cond_image_1d_tmp[:,3:,items["idx"][0]] =  items["val"][:,items["idx"][0]].clone()*1.0
        ...
        #     cond_image_1d_tmp[:,3:,:cond_len]= items["data"][3:,items["idx"][0]].detach().clone()*0.0
