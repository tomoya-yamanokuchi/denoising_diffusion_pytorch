...
if __name__ == '__main__':
    ...
    tags = "0"
    dataset_path   = f"/home/haxhi/dataset/denoising_diffusion_pytorch/dataset/real_models/SheetSander_kwon/random_generation_for_test/samples/samples_{tags}/"
    config_name    = f"SheetSander_type{tags}.yaml"
    ...
    plotter = BackgroundPlotter(show = True,
            window_size=(1920, 1920), title=viewer_title)
    ...
    for idx, val in enumerate(model_config):
        for idx, vals in enumerate(model_config[val]):
            ...
            if val == "outer_parts":
                plotter.add_mesh(mesh, opacity=0.4, color = color,show_edges=False,)
            else:
    ...
    save_name = os.path.basename(os.path.normpath(dataset_path))
    pil_image.save(f"./{save_name}.png")
