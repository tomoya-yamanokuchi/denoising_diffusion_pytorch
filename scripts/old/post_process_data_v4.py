...
def color_range_mask_segmentation(image, mask_config , base_image):
    # スケーリング（0-255画像のとき）
    if image.dtype == np.uint8:
        image = image.astype(np.float32) / 255.0

    mask_value = mask_config["target_mask"]
    ...
    base_image[mask, :] = mask_value

    return base_image

def get_segmented_pixel_loss(target_image,pred_image,condition_image):
    ...
    mask_config ={   "image_mask_config_b" : {"target_mask"  :np.asarray([0.2,0.8,0.8]),
                    ...
                    "image_mask_config_r": {"target_mask"   :np.asarray([0.8,0.2,0.2]),
                                            "target_mask_lb":np.asarray([0.8,0.2,0.2])-np.asarray([0.25,0.25,0.25]),
                                            "target_mask_ub":np.asarray([0.8,0.2,0.2])+np.asarray([0.25,0.25,0.25])}, #np.asarray([0.2,0.6,0.6])
                    "image_mask_config_y": {"target_mask"   :np.asarray([0.8,0.8,0.2]),
                                            "target_mask_lb":np.asarray([0.8,0.8,0.2])-np.asarray([0.25,0.25,0.25]),
                                            "target_mask_ub":np.asarray([0.8,0.8,0.2])+np.asarray([0.25,0.25,0.25])}}
    # image = np.zeros_like(target_image, dtype=np.float32)
    # pred_image_segmented_1 = color_range_mask_segmentation(image=pred_image, mask_config = mask_config["image_mask_config_b"],base_image=image)
    # pred_image_segmented_2 = color_range_mask_segmentation(image=pred_image, mask_config = mask_config["image_mask_config_r"],base_image=image)
    # pred_image_segmented_3 = color_range_mask_segmentation(image=pred_image, mask_config = mask_config["image_mask_config_y"],base_image=image)
    
    # 空画像を作成（pred_imageと同サイズ）
    masked_image = np.zeros_like(pred_image, dtype=np.float32)+1.0
    # 3種類のマスクを順番に適用
    for key in mask_config:
        masked_image = color_range_mask_segmentation(pred_image, mask_config[key], masked_image)

    
    
    target_image    = target_image*(condition_image<=0)
    pred_image      = masked_image*(condition_image<=0)

    # un_condition_mask = np.where(condition_image==0.0)[0].shape[0]/condition_image.shape[2]
    un_condition_mask = np.sum(np.all(condition_image == 0.0, axis=2))
    ...
    normarized_loss = loss/un_condition_mask

    if not np.where(condition_image==0.0)[0].shape[0]/condition_image.shape[2] == np.sum(np.all(condition_image == 0.0, axis=2)):
        import ipdb;ipdb.set_trace()



    return loss, normarized_loss
...
if __name__ == '__main__':
        ...
        for j in range(len(episode_folders)): # episode folder loop
            ...
            if args.policy_config["ctrl_mode"] == "oracle_obs" or args.policy_config["ctrl_mode"] == "random" :
                ...
                for k in range(len(condition_image_path_z)):
                    ...
                    augnet_loss_r.append((augnet_loss*255/un_condition_mask)*1000) # augnet_loss_r.append(augnet_loss)
                ...
                for k in tqdm(range(len(condition_image_path_z))):
                    ...
                    for p in range(len(ensemble_image_path)):
                        ...
                        segmented_pixel_loss, segmented_pixel_loss_normalized = get_segmented_pixel_loss(target_image = oracle_image, pred_image = ensemble_image, condition_image=condition_image)
                        ...
                        augnet_loss_r_tmp.append(segmented_pixel_loss_normalized)
                        augnet_loss_r_tmp_2.append(segmented_pixel_loss)
    ...
    with open(save_folders+"/post_processed_data_v6.json", mode="w") as f:
